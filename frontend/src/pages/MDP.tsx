...
export default function MDP() {
  ...
  const runPredict = async (pos: LatLng) => {
    ...
    try {
      ...
      const cat = Number.isFinite(data?.category)
        ? Math.trunc(data.category)
        : 1;

      const category = Math.min(7, Math.max(1, cat));

      const features = Array.isArray(data?.features)
        ? data.features.slice(0, 7).map((v) => {
            const n = Number(v) || 0;
      ...
      const probabilities = Array.isArray(data?.probabilities)
        ? data.probabilities.slice(0, 7).map((v) => {
            const n = Number(v) || 0;
            return Math.max(0, Math.min(1, n));
          })
        : undefined;
      ...
      if (mounted.current) {
        ...
        setPrediction({
          ...
          probabilities: Array.from({ length: 7 }, () => Math.random()),
        });
  ...
  const currentColor = prediction
    ? CATEGORY_COLORS[Math.max(0, Math.min(6, prediction.category - 1))]
    : "#0ea5e9";
        ...
        <aside className="w-full lg:w-96 bg-white dark:bg-slate-900 rounded-md shadow p-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="font-mono">
              {clickPos
                ? `${clickPos.lat.toFixed(6)}, ${clickPos.lng.toFixed(6)}`
                : "Click on the map"}
            </div>
          </div>
          ...
          <div>
              ...
              <div className="text-sm">
                {prediction
                  ? [
                      ...
                      "Very High",
                      "Critical",
                    ][Math.max(0, Math.min(6, prediction.category - 1))] ??
                    "Unknown"
                  : "—"}
              </div>
            </div>
            ...
            {error && (
              <div className="text-xs text-amber-600 mt-2">{error}</div>
            )}
          </div>
            ...
            <div className="mt-2 space-y-3">
              {prediction ? (
                ...
                <div className="text-sm text-slate-500">
                  No prediction yet — click on the map to run model
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() => clickPos && runPredict(clickPos)}
              disabled={!clickPos || loading}
          ...
          <div className="text-xs text-slate-400">
            Tip: map click sends lat/lng to your backend at{" "}
            <code>/api/predict</code>. Response shape:{" "}
            <code>{`{category:number, features:number[7]}`}</code>
          </div>
        </aside>
      </div>
    </>
